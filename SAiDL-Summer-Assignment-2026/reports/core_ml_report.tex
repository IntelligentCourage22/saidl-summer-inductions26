...
To directly test whether the strongest positional encodings generalize beyond
their training context, I trained GQA + ALiBi and GQA + RoPE with sequence
length 512 and evaluated their final checkpoints at longer validation contexts.
...
\bottomrule
\end{tabular}
\caption{Length extrapolation for GQA models trained at context length 512.}
\end{table}
...
consistent with ALiBi's design: the positional signal is computed as a
distance-dependent attention bias rather than relying on absolute rotary phases
...
from 159.20 perplexity at 512 tokens to 280.24 at 2048 tokens. This separates
in-context validation quality from length generalization: ALiBi is not merely
the best positional encoding at the training length, but also the most
convincing choice for extrapolating beyond it.
