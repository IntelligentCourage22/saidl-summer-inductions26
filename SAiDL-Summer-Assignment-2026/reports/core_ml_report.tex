...
\toprule
Assignment Component & Coverage \\
\midrule
Baseline Transformer & Standard causal Transformer trained at context length 1024. \\
Attention variants & Standard, sliding-window/local, linear kernelized, MQA, and GQA are implemented; standard, sliding-window, linear, and GQA are compared at 1024 tokens for perplexity and at 512/1024/2048 tokens for latency and memory scaling. \\
Positional encodings & Sinusoidal, RoPE, ALiBi, and learned relative position bias are implemented and evaluated. \\
Extrapolation & Sinusoidal, RoPE, ALiBi, and relative-bias models trained at 512 tokens and evaluated at 512, 1024, and 2048 tokens. \\
Hybrid blocks & Conv-before-attention, interleaved convolution, and gated-convolution FFN are implemented; conv-before and gated-conv are evaluated with the best GQA + ALiBi configuration. \\
Logging & JSONL/W\&B logging, validation perplexity, throughput, peak memory, epoch timing, gradient norm, and final metric artifacts are included. \\
\bottomrule
\end{tabular}
\caption{Core ML requirement coverage.}
\end{table}
...
The assignment suggests evaluating attention variants across multiple context
lengths such as 512, 1024, 2048, and 4096 where feasible. Under the available
compute budget, I prioritized full training comparisons at the main 1024-token
context, then measured forward-pass latency and memory scaling for the main
attention variants at 512, 1024, and 2048 tokens. A full retraining grid over
every attention variant and every context length is therefore left as a
compute-bound extension rather than a missing architectural component.
...
training the positional variants at $L_{\mathrm{train}}=512$ and evaluating them
at $L_{\mathrm{test}}\in\{512,1024,2048\}$.
...
I also benchmarked the main attention variants across sequence lengths to
quantify the cost of longer-context evaluation. These measurements use dense
forward passes with batch size 2 on the same GPU environment and are intended
to compare architectural scaling rather than cached autoregressive decoding.
...
\midrule
Standard & 512 & 33068.5 & 30.97 & 382.0 \\
Standard & 1024 & 30230.8 & 67.75 & 581.1 \\
Standard & 2048 & 23811.6 & 172.02 & 981.5 \\
Sliding Window & 512 & 34221.8 & 29.92 & 382.0 \\
Sliding Window & 1024 & 28788.1 & 71.14 & 581.1 \\
Sliding Window & 2048 & 22128.3 & 185.10 & 1007.5 \\
Linear & 512 & 22563.5 & 45.38 & 577.7 \\
Linear & 1024 & 22390.7 & 91.47 & 972.5 \\
Linear & 2048 & 22307.9 & 183.61 & 1763.5 \\
GQA + ALiBi & 512 & 31897.5 & 32.10 & 369.0 \\
GQA + ALiBi & 1024 & 27673.8 & 74.01 & 568.1 \\
GQA + ALiBi & 2048 & 20359.0 & 201.19 & 968.5 \\
\bottomrule
\end{tabular}
\caption{Inference latency benchmark across attention variants with batch size 2. Standard, sliding-window, and linear rows use sinusoidal positions; the final selected model uses GQA + ALiBi.}
\end{table}

The benchmark shows the expected growth in latency and memory with sequence
length. Standard attention is fastest in this forward-only benchmark at 2048
tokens, while GQA + ALiBi uses fewer parameters and slightly less memory. Linear
attention has almost flat tokens/sec across sequence lengths, but its current
implementation is memory-heavy because the cumulative kernel statistics are
materialized with an additional head-dimension product. Sliding-window attention
is competitive at 512 tokens but does not improve the 2048-token forward-pass
latency in this implementation. Taken together with the validation perplexity
results, the selected GQA + ALiBi model remains the best quality-oriented final
choice, even though standard attention remains a strong raw-throughput baseline.
...
tested budget, and the logged run should be interpreted as preliminary because
it was trained for fewer steps than the other variants. The current
implementation computes local key/value windows directly rather than
materializing a full attention matrix, so its attention-score memory scales with
sequence length times window size. Linear attention improved validation
perplexity slightly over the standard baseline but required substantially more
training memory and reduced throughput, which weakens its appeal as a practical
replacement.
...
same codebase should add a fuller multi-context retraining matrix, especially at
4096 tokens if compute allows. The current results already support GQA + ALiBi
as the final Core ML submission model.
