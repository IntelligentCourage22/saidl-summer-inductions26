...
Attention variants & Standard, sliding-window/local, linear kernelized, MQA, and GQA are implemented; standard, sliding-window, linear, and GQA are evaluated at 1024 tokens. \\
...
same codebase should add comparative latency baselines for every attention type
and a fuller multi-context retraining matrix, especially at 4096 tokens if
compute allows. The current results already support GQA + ALiBi as the final
Core ML submission model.
