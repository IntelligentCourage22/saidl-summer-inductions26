...
To directly test whether the strongest positional encoding generalizes beyond
its training context, I trained GQA + ALiBi with sequence length 512 and
evaluated the final checkpoint at longer validation contexts.

\begin{table}[h]
\centering
\begin{tabular}{lrrrr}
\toprule
Train Context & Eval Context & Val Loss & Val PPL & Eval Time (s) \\
\midrule
512 & 512 & 5.0253 & 152.21 & 8.17 \\
512 & 1024 & 5.0033 & 148.90 & 9.91 \\
512 & 2048 & 4.9956 & 147.76 & 13.29 \\
\bottomrule
\end{tabular}
\caption{Length extrapolation for GQA + ALiBi trained at context length 512.}
\end{table}

The extrapolation result is strong: validation perplexity remains stable when
the evaluation context is extended beyond the training length, and in this run
it slightly improves from 152.21 at 512 tokens to 147.76 at 2048 tokens. This
is consistent with ALiBi's design, since the positional bias is computed as a
distance-dependent attention bias rather than read from an absolute position
table tied to the training context. The result supports using GQA + ALiBi as the
final Core ML architecture, especially when length extrapolation is part of the
...
context. This is an important result because it separates in-context validation
quality from length generalization: ALiBi is not merely the best positional
encoding at the training length, but also the most convincing choice for
